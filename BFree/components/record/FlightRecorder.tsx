				...
				try {
					...
					console.log(
						'tick! ' +
							...
							// `alt: ${loggedAlt} ` +
							`speed: ${speed ? speed.speed : 'N/A'} `
					);
					// Publish speed to MQTT
					// TODO We should probably publish all the measurements and not just speed.
					if (speed.speed !== undefined) {
						// TODO We should probably have a topic structure like `homeTrainer/sensor/speed` or so.
						// This would allow users to subscribe to only the sensors they are interested in.
						// We should also probably publish a message when the recording starts with the list of
						// available sensors and their units.
						publishMessage('homeTrainerCastres/Groupe1B/speed', {
							speed: speed.speed,
							timestamp: Date.now(),
						});
					}
